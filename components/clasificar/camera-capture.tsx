...
const ICON_BTN =
  'grid h-10 w-10 place-items-center rounded-full bg-wine/85 text-cream shadow-soft backdrop-blur transition-colors hover:bg-wine';
...
export function CameraCapture({ onCapture }: Props) {
  ...
  return (
    <div className="relative aspect-[4/3] overflow-hidden rounded-3xl border-2 border-wine/20 bg-ink">
      ...
      {/* Feed en vivo: presente mientras se pide o transmite la cámara. */}
      ...
      <AnimatePresence mode="wait">
        {/* Estado inicial: invitar a encender la cámara. */}
        {status === 'idle' && (
          <motion.div
            key="idle"
            initial={{ opacity: 0 }}
            ...
            className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-canvas/40 px-6 text-center"
          >
            ...
            <button
              ...
              className="text-[12px] font-semibold text-wine/70 underline-offset-4 transition-colors hover:text-wine hover:underline"
            >
        ...
        {/* Pidiendo permiso / abriendo dispositivo. */}
        {status === 'requesting' && (
          <motion.div
            key="requesting"
            role="status"
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.25 }}
            className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-ink/70 px-6 text-center"
          >
            <div className="flex items-center gap-2" aria-hidden="true">
              {[0, 1, 2].map((i) => (
                <motion.span
                  key={i}
                  className="h-2.5 w-2.5 rounded-full bg-cream"
        ...
        {/* Transmitiendo: controles sobre el video. */}
        {status === 'streaming' && (
          <motion.div
            key="streaming"
            role="status"
          ...
          >
            ...
            <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-ink/45 via-transparent to-ink/25" />

            <div className="absolute left-3 right-3 top-3 flex items-center justify-between">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-cream/90 px-3 py-1 font-mono text-[10px] uppercase tracking-[0.22em] text-wine">
            ...
            <div className="absolute inset-x-0 bottom-5 flex flex-col items-center gap-2">
              {!ready && (
                <span className="rounded-full bg-ink/70 px-3 py-1 font-mono text-[10px] uppercase tracking-[0.18em] text-cream">
              ...
              <button
                ...
                className="group grid h-[68px] w-[68px] place-items-center rounded-full border-4 border-cream bg-cream/25 backdrop-blur transition-transform duration-150 active:scale-90 disabled:cursor-not-allowed disabled:opacity-50 disabled:active:scale-100"
              >
                <span className="h-12 w-12 rounded-full bg-cream transition-colors group-hover:bg-olive group-disabled:bg-cream/70" />
              </button>
            </div>
          </motion.div>
        )}

        {/* Error: mensaje claro + reintento + fallback nativo. */}
        {status === 'error' && (
          <motion.div
            key="error"
            role="alert"
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.25 }}
            className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-canvas/50 px-6 text-center"
          >
            <div className="grid h-14 w-14 place-items-center rounded-2xl bg-red-100 text-2xl" aria-hidden="true">
              📷
            </div>
      ...
      <AnimatePresence>
        {hint && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 8 }}
            transition={{ duration: 0.25 }}
            className="absolute inset-x-3 top-16 z-30 rounded-xl bg-wine/90 px-3 py-2 text-center text-[12px] font-medium text-cream backdrop-blur"
          >
            {hint}
          </motion.div>
        )}
